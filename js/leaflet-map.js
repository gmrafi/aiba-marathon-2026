(function(){
  ...
  function turnaroundIndex(subset, fullSet){
    // Returns the index in subset that is farthest from the start
    if(!subset||subset.length<2) return 0;
    var start=subset[0],furthestIdx=subset.length-1,maxD=0;
    for(var i=1;i<subset.length-1;i++){
      var d=Math.hypot(subset[i][0]-start[0],subset[i][1]-start[1]);
      if(d>maxD){maxD=d;furthestIdx=i;}
    }
    return furthestIdx;
  }
  function applyRoutePaint(map,subset){
    var label=(map._legendLabel||'half');
    var main= label==='kids'?colors.routeKids : label==='10k'?colors.route10k : colors.routeHalf;
    if(map._routeLayers.route&&map._routeLayers.route.setStyle){
      map._routeLayers.route.setStyle({color:main});
      map._routeLayers.shadow.setStyle({color:colors.outline});
    }
    if(map._turnMarker){ map.removeLayer(map._turnMarker); }
    var idx=turnaroundIndex(subset);
    var tIcon=L.divIcon({
      className:'turn-marker',
      html:'<div class="turn-pin"><span>U-TURN</span></div>',
      iconSize:[78,40],iconAnchor:[39,32]
    });
    map._turnMarker=L.marker(subset[idx],{icon:tIcon,zIndexOffset:980,interactive:true})
      .bindPopup('<strong>Turnaround Point</strong><br/><small>Run turns back here toward Army IBA start/finish</small>')
      .addTo(map);
  }
  ...
  function init(el,data){
    ...
    el._legendLabel='half';
    applyRoutePaint(map, coords);
    ...
    addOfficialMarkersSet(el._leafletMap, el, 'half');
    return map;
  }
    ...
    document.querySelectorAll('[data-route]').forEach(function(btn){
      btn.addEventListener('click', function(){
        ...
        var labelText = label==='kids'
          ? 'Kids 1K Fun Run · Army IBA ↔ Short loop turnaround'
          : label==='10k'
          ? '10K Marathon · Army IBA ↔ Academia Square Sreerampur Sylhet (Full Hydration turnaround)'
          : 'Half Marathon 21.1 KM · Army IBA ↔ Gotatikor Point turnaround';
        var km = label==='kids' ? 1.0 : label==='10k' ? 10.0 : 21.1;
        var main = label==='kids' ? colors.routeKids : label==='10k' ? colors.route10k : colors.routeHalf;
        document.querySelectorAll('.race-map').forEach(function(el){
          ...
          var out = coords;
          var halfN = Math.floor(out.length*0.49);
          var tenN  = Math.floor(out.length*0.225);
          var sub;
          if (label==='kids') {
            sub = out.slice(0, Math.max(8, Math.floor(out.length*0.04))+1);
          } else if (label==='10k') {
            var forward = out.slice(0, Math.max(8, tenN)+1);
            sub = forward.concat(forward.slice(0,-1).reverse());
          } else {
            var forwardH = out.slice(0, Math.max(8, halfN)+1);
            sub = forwardH.concat(forwardH.slice(0,-1).reverse());
          }
          el._routeLayers.route.setLatLngs(sub);
          el._routeLayers.shadow.setLatLngs(sub);
          m.fitBounds(L.latLngBounds(sub), {padding: [40,40]});
          addOfficialMarkersSet(m, el, label);
        });
        var n = document.getElementById('mapRouteName');
        var d = document.getElementById('mapDistance');
        if(n) n.textContent = labelText;
        if(d) d.textContent = km.toFixed(1)+' KM (~'+(km*5.5).toFixed(0)+' min runner)';
      });
    });
    var fly = document.getElementById('flyRoute');
    if(fly) fly.onclick = function(){
      document.querySelectorAll('.race-map').forEach(function(el){
        if(el._leafletMap && el._routeLayers) el._leafletMap.fitBounds(el._routeLayers.bounds,{padding:[40,40],animate:true,duration:1.2});
      });
    };
  }
  ...
  function addOfficialMarkersSet(map, el, label){
    ...
    var attr = label==='kids' ? 'data-official-kids' : (label==='10k' ? 'data-official-10k' : 'data-official-half');
    var raw = el.getAttribute(attr);
    if (!raw) return;
    raw.split('::').forEach(function(spec){
      if(!spec) return;
      var p = spec.split('|');
      var lat=parseFloat(p[0]), lng=parseFloat(p[1]), kind=p[2], km=p[3]||'', lab=p[4]||'';
      var html = (kind==='start'?'S':kind==='water'?'W':kind==='turn'?'↧':'+');
      var marker = L.marker([lat,lng],{
        icon:L.divIcon({className:'mco-pin mco-pin-'+kind, html:'<div>'+html+'</div>', iconSize: kind==='turn'?[40,28]:[30,30], iconAnchor: kind==='turn'?[20,14]:[15,15]})
      }).addTo(map);
      var pop = '<strong>'+lab+'</strong>'+(km?'<br><small style="opacity:.85">'+km+' km</small>':'');
      marker.bindPopup(pop, {closeButton:true, maxWidth:260});
      el._officialMarkers.push(marker);
    });
  }
...
(function injectTurnCSS(){
  if(document.getElementById('lf-turn-style'))return;
  var s=document.createElement('style');s.id='lf-turn-style';
  s.textContent='.turn-marker .turn-pin{background:#ffb84d;color:#1a0d00;border:2px solid #fff;border-radius:10px;padding:4px 8px;font:700 11px Oswald,sans-serif;letter-spacing:.08em;text-align:center;box-shadow:0 4px 14px rgba(0,0,0,.45);text-transform:uppercase;}'
    +'.turn-marker .turn-pin span{display:inline-block;}'
    +'.turn-marker .turn-pin::before{content:"\21B7";margin-right:6px;font-weight:700;}';
