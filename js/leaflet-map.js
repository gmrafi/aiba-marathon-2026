(function(){
  ...
  function refreshButtons(){
    document.querySelectorAll('[data-route]').forEach(function(btn){btn.addEventListener('click',function(){
      ...
      var labelText=label==='kids'?'Kids 1K Fun Run · Army IBA ↔ AIBA-loop turn':label==='10k'?'Students Run 10 KM · Army IBA ↔ AcademySq boulevard turn':'Half Marathon 21.1 KM · Army IBA ↔ Midpoint turn';
      ...
      document.querySelectorAll('.race-map').forEach(function(el){var m=el._leafletMap;if(!m||!el._routeLayers)return;
        ...
        applyRoutePaint(m,subset);
        m.fitBounds(L.latLngBounds(subset),{padding:[28,28]});
      });
      var n=document.getElementById('mapRouteName'),d=document.getElementById('mapDistance');
      if(n)n.textContent=labelText;
      if(d)d.textContent=km.toFixed(1)+' KM (~'+(km*5.5).toFixed(0)+' min runner)';
      var note=document.getElementById('mapDemoNote');
      if(note)note.textContent='OUT & BACK · START/FINISH AT ARMY IBA';
    });});
